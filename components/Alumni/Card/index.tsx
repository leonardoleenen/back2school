  ...
  return (
    <div key={props.alumni.id}>
      <article className="bg-gray-200 border border-gray-300 rounded-lg shadow-md text-center mb-4 p-4 mt-12">
        <img src={almuni.photo} className="m-auto w-16 h-16 -mt-12 rounded-full block shadow-base" />
        ...
        <button onClick={() => selectAlumni(almuni)} className="text-sm font-semibold w-full bg-indigo-500 rounded-lg p-2 text-white mt-4">Informar ausentismo</button>
        <button className="text-sm font-semibold w-full bg-indigo-200 rounded-lg p-2 text-indigo-500 mt-4">Ausentismo informado</button>
      </article>
    </div>
  )
}
